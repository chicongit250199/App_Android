...
public class XMPPConnection extends Connection {
    ...
    public synchronized void login(String username, String password, String resource) throws XMPPException {
        ...
        if (this.roster == null) {
            this.roster = new Roster(this, false);
        }
        if (this.reversedRoster == null) {
        	this.reversedRoster = new Roster(this, true);
        }
        
        if (config.isRosterLoadedAtLogin()) {
            this.roster.reload(false);
            this.reversedRoster.reload(true);
        }
    ...
    public Roster getRoster(boolean reversed) {
    	if (reversed) {
    		return getReversedRoster();
    	} else {
    		return getNormalRoster();
    	}
    }
    
    private Roster getNormalRoster () {
...
//        }
    ...
    private Roster getReversedRoster () {
        // synchronize against login()
        synchronized(this) {
            // if connection is authenticated the roster is already set by login() 
            // or a previous call to getRoster()
            if (!isAuthenticated() || isAnonymous()) {
                if (reversedRoster == null) {
                	reversedRoster = new Roster(this, true);
                }
                return reversedRoster;
            }
        }

//        if (!config.isRosterLoadedAtLogin()) {
        reversedRoster.reload(true);
//        }
        // If this is the first time the user has asked for the roster after calling
        // login, we want to wait for the server to send back the user's roster. This
        // behavior shields API users from having to worry about the fact that roster
        ...
        // Smack code should be wary about calling the getRoster method, and may need to
        // access the roster object directly.
        if (!reversedRoster.rosterInitialized) {
            try {
                synchronized (reversedRoster) {
                    long waitTime = SmackConfiguration.getPacketReplyTimeout();
                    long start = System.currentTimeMillis();
                    while (!reversedRoster.rosterInitialized) {
                        if (waitTime <= 0) {
                            break;
                        }
                        reversedRoster.wait(waitTime);
                        long now = System.currentTimeMillis();
                        waitTime -= now - start;
                        start = now;
                    }
