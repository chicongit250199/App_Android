...
public class PacketParserUtils {
    ...
    public static Packet parseMessage(XmlPullParser parser) throws Exception {
        ...
        message.setWith(parser.getAttributeValue("", "with"));
        message.setType(Message.Type.fromString(parser.getAttributeValue("", "type")));
        message.setSubtype(parser.getAttributeValue("", "subtype"));
        message.setAck(parser.getAttributeValue("", "ack"));
        message.setDest(parser.getAttributeValue("", "dest"));
        message.setTimestamp(parser.getAttributeValue("", "timestamp"));
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                if (elementName.equals("subject")) {
                ...
                else if (elementName.equals("expandbody")) {
                    String expandbody = parseContent(parser);
                    Log.i("Hoa debug","PacketParserUtils:parseMessage:expandbody = "+ expandbody);
                    message.addExpandBody(expandbody);
                }
                    ...
                    if (message.getExtension("avatar") == null) {
                    ...
                    message.setFile(parseAntBuddyFile(parser));
                }
    ...
    public static IQ parseIQ(XmlPullParser parser, Connection connection) throws Exception {
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                else if (elementName.equals("query") && (namespace.equals("jabber:iq:roster") || namespace.equals("jabber:iq:reverseroster"))) {
                    iqPacket = parseRoster(parser);
                } else if (elementName.equals("query") && namespace.equals("jabber:iq:publicperm:roster")) {
                	iqPacket = parsePublicPerm(parser);
                } else if (elementName.equals("query") && namespace.equals("secret_roster")) {
                	iqPacket = parseSecretRoster(parser);
                } else if (elementName.equals("query") && namespace.equals("jabber:iq:register")) {
                    iqPacket = parseRegistration(parser);
                } else if (elementName.equals("query") && namespace.equals("perm_notf")) {
                	iqPacket = new PermNotiPacket();
    ...
    private static PublicNotificationPacket parsePublicPerm(XmlPullParser parser) throws Exception {
    	
        PublicNotificationPacket roster = new PublicNotificationPacket();
        boolean done = false;
        PublicNotificationPacket.Item item = null;
        while (!done) {
            int eventType = parser.next();
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals("item")) {
                	// Create packet.
                    item = new PublicNotificationPacket.Item();
                    
                    item.userJid = parser.getAttributeValue("", "jid");
                    item.username = parser.getAttributeValue("", "name");
                    item.firstname = parser.getAttributeValue("", "firstname");
                    item.lastname = parser.getAttributeValue("", "lastname");
                    item.lastContacted = parser.getAttributeValue("", "contacted");
                    item.url1 = parser.getAttributeValue("", "url1");
                    item.url2 = parser.getAttributeValue("", "url2");
                    item.notifyPublicCall = parser.getAttributeValue("", "p_pub_call");
                    item.notifyAppMinimized = parser.getAttributeValue("", "p_pub_app_min");
                    item.notifyAppOpened = parser.getAttributeValue("", "p_pub_app_open");                    
                }                
            }
            else if (eventType == XmlPullParser.END_TAG) {
                if (parser.getName().equals("item")) {
    ...
    private static RosterPacket parseRoster(XmlPullParser parser) throws Exception {
    	...
    	if (namespace.equals("jabber:iq:reverseroster")) {
    		reversed = true;
    	}
    	
        RosterPacket roster = new RosterPacket(reversed);
        boolean done = false;
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals("item")) {
                    ...
                    //custom roster vales added oncam
                    String fromMe = parser.getAttributeValue("", "fromme");
                    item.setFromMe(fromMe);
                    String fromRoster = parser.getAttributeValue("", "fromroster");
                    item.setFromRoster(fromRoster);
                    String lastContacted = parser.getAttributeValue("", "contacted");
                    item.setLastContacted(lastContacted);
                    String url1 = parser.getAttributeValue("", "url1");
                    item.setUrl1(url1);
                    String url2 = parser.getAttributeValue("", "url2");
                    item.setUrl2(url2);
                    
                    String firstName = parser.getAttributeValue("", "firstname");
                    item.setFirstName(firstName);
                    String lastName = parser.getAttributeValue("", "lastname");
                    ...
                    String privCall = parser.getAttributeValue("", "p_priv_call");
                    item.setPrivCall(privCall);
                    
                    String dropin = parser.getAttributeValue("", "p_dropin");
                    item.setDropin(dropin);
                }
        ...
        Log.i("reverse", roster.toXML());
        
        return roster;
    }
    
    private static SecretRoster parseSecretRoster(XmlPullParser parser) throws Exception {
    	
    	SecretRoster roster = new SecretRoster();
        boolean done = false;
        SecretRoster.Item item = null;
        while (!done) {
            int eventType = parser.next();
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals("item")) {
                    ...
                    String name = parser.getAttributeValue("", "name");
                    // Create packet.
                    item = new SecretRoster.Item(jid, name);
                    ...
                    String privateCall = parser.getAttributeValue("", "p_priv_call");
                    String dropinCall = parser.getAttributeValue("", "p_dropin");
                    
                    item.privCall = privateCall;
                    item.dropin = dropinCall;
                    // Set status.
//                    String ask = parser.getAttributeValue("", "ask");
//                    RosterPacket.ItemStatus status = RosterPacket.ItemStatus.fromString(ask);
//                    item.setItemStatus(status);
...
//                    item.setItemType(type);
     ...
     * @throws Exception if an exception occurs while parsing the packet.
     */
    public static AntBuddyFile parseAntBuddyFile(XmlPullParser parser) throws Exception {
        String name = "";
        int size = 0;
        String fileUrl = null;
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                String elementName = parser.getName();
                if (elementName.equals("name")) {
                    name = parseContent(parser);
                }
                else if (elementName.equals("size")) {
                    size = Integer.valueOf(parseContent(parser));
                }
                else if (elementName.equals("fileUrl")) {
                    fileUrl = parseContent(parser);
                }
                else if (elementName.equals("mimeType")) {
                    mimeType = parseContent(parser);
                }
                else if (elementName.equals("thumbnailUrl")) {
                    thumbnailUrl = parseContent(parser);
                }
            }
            else if (eventType == XmlPullParser.END_TAG) {
        ...
        return new AntBuddyFile(name, size, fileUrl, mimeType, thumbnailUrl);
    }
