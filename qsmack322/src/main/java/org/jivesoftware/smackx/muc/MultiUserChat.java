...
public class MultiUserChat {
    ...
    public synchronized void join(
        ...
        try {
            ...
            // Send join packet.
            Log.i("JoinPacket", joinPresence.toXML());
            connection.sendPacket(joinPresence);
            // Wait up to a certain number of seconds for a reply.
     ...
     */
    public String sendConfigurationForm(Form form) throws XMPPException {
        MUCOwner iq = new MUCOwner();
        iq.setTo(room);
    ...
    public void destroy(String reason, String alternateJID) throws XMPPException {
        ...
        Log.i("iq destroy request", iq.toXML());
        connection.sendPacket(iq);
        // Wait up to a certain number of seconds for a reply.
        IQ answer = (IQ) response.nextResult(SmackConfiguration.getPacketReplyTimeout());
        // Stop queuing results
    ...
    private void changeAffiliationByAdmin(String jid, String affiliation, String reason)
        ...
        PacketCollector response = connection.createPacketCollector(responseFilter);
        // Send the change request to the server.
        android.util.Log.i("iq request", iq.toXML());
        connection.sendPacket(iq);
        // Wait up to a certain number of seconds for a reply.
        IQ answer = (IQ) response.nextResult(SmackConfiguration.getPacketReplyTimeout());
        // Stop queuing results
