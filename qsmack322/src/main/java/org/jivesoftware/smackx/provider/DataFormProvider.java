...
public class DataFormProvider implements PacketExtensionProvider {
    ...
    private FormField parseField(XmlPullParser parser) throws Exception {
        ...
        formField.setCallType(parser.getAttributeValue("", "calltype"));
        formField.setCallId(parser.getAttributeValue("", "callid"));
        formField.setInviteType(parser.getAttributeValue("", "invitetype"));
        formField.setEndAction(parser.getAttributeValue("", "endaction"));
        while (!done) {
            int eventType = parser.next();
            if (eventType == XmlPullParser.START_TAG) {
